...
const gravity3d = function (id) {
  ...
  camera.position.set(0, 0, settings.width * 4);
  ...
  for (let i = 0; i < 7; i++) {
    index = mass.length
    mass[index] = new THREE.Mesh(geometry, material);
    mass[index].radius = radius + 0.5 * (Math.random() - 0.5)
    mass[index].mass = radius / 10
    mass[index].position.set(35 + i * 10, 0, 0);
    mass[index].velocity = {
      x: 0.03 * (Math.random() - 0.5),
      y: 0.2 + 0.01 * (Math.random() - 0.5),
